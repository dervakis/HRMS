...
export const getTravelDocumentRequest = async({planId, userId}:{planId:number,userId:number}): Promise<EmployeeTravelDocumentType[]> =>{
    const response = await Api.get(`/document/request/${planId}/${userId}`);
